...
def make_predictions(im, model, scales=[50], stride = 10):
    ...
    for scale in scales:
        for y1 in range(0, im.shape[0] - scale, stride):
            for x1 in range(0, im.shape[1] - scale, stride):
                x2 = x1 + scale
                y2 = y1 + scale
                curr = im[y1:y2, x1:x2,:]
                curr = cv2.resize(curr, (64, 64))
                curr = normalize_data_per_row(np.array([curr]))
                prob = model.predict(curr).item() # collapse singleton into a numerical val
                if prob > 0.4: # use 0.4 as the best threshold
                    boxes.append([x1, y1, x2, y2, prob])
    boxes = np.array(boxes)
    return non_max_suppression(boxes, 0.25)
